#############################################################################
# get.oc.BOINETC_m3_v02.R: function to calculate operating characteristics of BOIN-ETC3
#############################################################################
get.oc.BOINETC_m3 <- function(ncohort, cohortsize, n.earlystop, ntrial,
  ...
  for (j in 1:(nd1-1)){
    if (nd1>j+1){dosespace <- append(dosespace, list(c(j*nd1+1:(nd1-j), ((j+2):nd2)*nd1-j)))}
    else {dosespace <- append(dosespace, list(nd1*(j:(nd2-1))+1))}
  }
  ...
  for (jjj in 1:ntrial){
    ...
    for (jj in 1:nd1){
      ...
      dspace <- dspace+1;
      if (l.selectdose == 1){startdose <- 1}
      else if (l.selectdose<=nd1-jj+1){startdose <- l.selectdose-1}
      else {startdose <- l.selectdose-(nd1-jj+1)}
